...
{//FontCombobox的响应，选择字体名称
    QTextCharFormat fmt;
    fmt.setFontFamilies({arg1});//设置字体名称
    ui->txtEdit->mergeCurrentCharFormat(fmt);
}
...
void QWMainWind::on_action_4_triggered()
{
    // 创建一个对话框用于显示信息
    QDialog *infoDialog = new QDialog(this);
    infoDialog->setWindowTitle("个人信息"); // 设置对话框标题
    infoDialog->resize(300, 150); // 设置对话框大小

    // 创建布局管理器，用于排列信息标签
    QVBoxLayout *layout = new QVBoxLayout(infoDialog);

    // 创建标签并设置显示内容（替换为你的姓名和学号）
    QLabel *nameLabel = new QLabel("姓名：李子诺", infoDialog);
    QLabel *idLabel = new QLabel("学号：2023414300208", infoDialog);
